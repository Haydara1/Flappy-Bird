...
func init_game_over_menu(score, highscore):
	...
	tween.interpolate_property(game_over_sprite, "modulate:a", 0, 1, 1.5)
	tween.start()
	yield(tween, "tween_completed")
	
	tween.interpolate_property(game_over_sprite, "modulate:a", 1, 0, 1.5)
	tween.start()
	yield(tween, "tween_completed")
